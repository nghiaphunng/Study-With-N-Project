...
@WebServlet("/GetFollowerListOfOtherUser")
public class GetFollowerListOfOtherUser extends HttpServlet {
	private static final long serialVersionUID = 1L;

    public GetFollowerListOfOtherUser() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {			
	        ...
	        List<User> followerList = FollowDAO.DisplayFollowerUserId(conn, userId);
			...
			ErrorDAO.insertError(conn, "messageErrorGetFollowerListOfOtherUser", e.getMessage());
		} finally {
			try {
				conn.close();
